...
public class AgentService {
    ...
    public AgentService(WebClient webClient, AppConfig config, SkillRegistry skillRegistry) {
    ...
    public String getSystemPrompt() {
        // 动态拼接当前注册的全部技能描述（含触发词/排除词），新增技能时系统提示自动更新，无需手动维护
        ...
        return """
                ...
                ## 你的能力范围
                1. 企业信息查询：查询企业基本信息、股东信息、受益人（实际控制人）、企业族谱、海关认证、海关失信、账户冻结、授信、人行账户管控等信息。
                2. 信息智能核实：辅助客户经理高效完成客户身份信息、经营资质、受益所有人等核心信息的采集、核验与交叉比对（核实营业执照等附件）。
                3. 风险识别与评级：对企业进行风险预查与风险筛查，识别重大风险事项，并按照风险程度形成风险分级。
                4. 尽调报告撰写：根据选择的模板以及提交的材料，生成尽调报告（含财务分析报告、授信评估报告等）。
                5. 历史尽调查询：根据企业名称查询历史尽调记录、报告列表。
                6. 信息缺口识别：主动指出材料缺失、数据矛盾、无法验证事项和需要进一步核实的问题。
                ## 可用技能
                以下是系统当前注册的全部技能。系统会自动完成意图识别并调用对应技能执行，技能执行结果会以结构化事件返回给你，你只需基于执行结果组织回复，无需也不要在回复中模拟技能调用：
                %s
                ## 技能路由说明
                1. 技能调用由系统的意图识别模块完成（确定性关键词匹配 + LLM 仲裁），你无需自行判断该调用哪个技能。
                2. 当用户意图模糊时，主动向用户澄清，一次只问 1-2 个问题。
                3. 当技能缺少必要参数（如企业名称）时，先向用户索要，不要臆造参数。
                ## 交互规范
                1. 始终保持专业、耐心、友好的态度
                ...
                9. 如果用户上传了附件但未说明用途（例如消息仅为"请查看我上传的附件"或只包含附件名），主动询问："您上传的附件是想用于信息核实，还是生成尽调报告？"；如果用户已明确说明用途（如包含"核实"、"生成报告"等意图词），则正常处理，不要询问""".formatted(skillsPrompt);
    }
